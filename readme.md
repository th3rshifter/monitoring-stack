<p align="center">
  <img src="https://raw.githubusercontent.com/th3rshifter/monitoring-stack/main/audit-api/1.jpg" width="24%" />
  <img src="https://raw.githubusercontent.com/th3rshifter/monitoring-stack/main/audit-api/2.jpg" width="24%" />
  <img src="https://raw.githubusercontent.com/th3rshifter/monitoring-stack/main/audit-api/3.jpg" width="24%" />
  <img src="https://raw.githubusercontent.com/th3rshifter/monitoring-stack/main/audit-api/4.jpg" width="24%" />
  <img src="https://raw.githubusercontent.com/th3rshifter/monitoring-stack/main/audit-api/5.jpg" width="24%" />
  <img src="https://raw.githubusercontent.com/th3rshifter/monitoring-stack/main/audit-api/6.jpg" width="24%" />
  <img src="https://raw.githubusercontent.com/th3rshifter/monitoring-stack/main/audit-api/7.jpg" width="24%" />
  <img src="https://raw.githubusercontent.com/th3rshifter/monitoring-stack/main/audit-api/8.jpg" width="24%" />
</p>


![Alt text for the image](https://github.com/th3rshifter/monitoring-stack/blob/main/audit-api/1.jpg)
![Alt text for the image](https://github.com/th3rshifter/monitoring-stack/blob/main/audit-api/2.jpg)
![Alt text for the image](https://github.com/th3rshifter/monitoring-stack/blob/main/audit-api/3.jpg)
![Alt text for the image](https://github.com/th3rshifter/monitoring-stack/blob/main/audit-api/4.jpg)
![Alt text for the image](https://github.com/th3rshifter/monitoring-stack/blob/main/audit-api/5.jpg)
![Alt text for the image](https://github.com/th3rshifter/monitoring-stack/blob/main/audit-api/6.jpg)
![Alt text for the image](https://github.com/th3rshifter/monitoring-stack/blob/main/audit-api/7.jpg)
![Alt text for the image](https://github.com/th3rshifter/monitoring-stack/blob/main/audit-api/8.jpg)

======
## 🧱 Используемый стэк

### Операционная система и инфраструктура
- Linux (Ubuntu Server)
- Docker (Compose)

### Базы данных
- PostgreSQL — централизованное хранение событий, инцидентов и отчётов аудита

### Мониторинг и оповещения
- Prometheus — сбор и хранение метрик
- Alertmanager — обработка и маршрутизация алертов
- Grafana — визуализация метрик и логов
- Node Exporter — сбор метрик состояния серверов
- cAdvisor — мониторинг Docker-контейнеров

### Централизованное логирование
- Loki — хранение и агрегация логов
- Promtail — сбор логов с серверов и контейнеров

### Источники событий безопасности и сети
- Fail2Ban — события информационной безопасности (подбор паролей, блокировки)
- Xray / X-UI — сетевые события и логи сервисов

---

## ⚙ Серверная часть и прикладной уровень

### Backend-фреймворк
- FastAPI — REST API и Web-интерфейс системы аудита и инцидентов
- Uvicorn — ASGI-сервер

### Доступ к базе данных
- SQLAlchemy (async) — асинхронная работа с PostgreSQL
- asyncpg — драйвер PostgreSQL

### Валидация данных и модели
- Pydantic v2 — валидация данных (Alertmanager, события)

### HTTP-интеграции
- httpx — асинхронные HTTP-запросы к Prometheus и Loki

### Шаблонизация и UI
- Jinja2 — серверный рендеринг HTML-интерфейса

### Формирование отчётов
- Pandas — подготовка данных для аналитических отчётов
- OpenPyXL — экспорт отчётов в формат Excel (XLSX)

---

## 🧩 Используемые модули стандартной библиотеки Python
- asyncio — асинхронное выполнение
- datetime — работа с временными интервалами и метками времени
- dataclasses — описание структур UI
- contextlib — управление жизненным циклом приложения
- io — генерация файлов в памяти
- json, os, typing, urllib.parse



Система предназначена для:
- централизованного сбора событий и логов ИТ-инфраструктуры;
- агрегации и корреляции инцидентов;
- анализа технического контекста инцидентов (метрики, логи);
- формирования отчётов для аудита и расследования инцидентов.
- устранение и анализ инцидента.

Проект реализует полный цикл обработки инцидента:  
**событие с ИС -> логирование -> корреляция -> анализ -> отчёт**.

---

## 🔄 Логика обработки инцидента

1. Prometheus фиксирует аномалию (CPU, память, диск, недоступность узла и т.д.).
2. Alertmanager формирует событие и отправляет его в Audit API.
3. Audit API:
   - сохраняет событие;
   - определяет инцидент (incident_key - номер инцидента);
   - собирает технический контекст за временное окно (1ч);
   - связывает метрики и логи;
   - формирует отчёт по инциденту.
4. Инцидент доступен:
   - через Web-интерфейс;
   - в виде JSON;
   - в формате Markdown;
   - в формате Excel (XLSX).
>>>>>>> 03b02cd3eae91464fefe77d37a0fc9d8053a634c
